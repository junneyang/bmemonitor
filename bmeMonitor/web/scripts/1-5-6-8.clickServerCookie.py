#-*- coding: utf-8 -*-
#-------------------------------------------------------------------------------
# Name:        cps clickServer cookie 监控
# Purpose:
#
# Author:      caolei07
#
        ...
        for item in data_list:
            ...
            if item.has_key('resCpsId') and item.has_key('expCpsId'):
                ...
                if str(item['expCpsId'])!=str(item['resCpsId']):
                    ...
                    self.warnList.append(dsc)
            elif not item.has_key('expCpsId'):
                dsc = '商家【'+str(item['name'])+'】：cpsid 不一致，请给出预期cpsid。'
                self.warnList.append(dsc)
            elif not item.has_key('resCpsId'):
                dsc = '商家【'+str(item['name'])+'】：cpsid 不一致，预期：'+str(item['expCpsId'])+"，实际返回无cpsid"
                self.warnList.append(dsc)
            ...
            if item.has_key('resCpsInfo') and item.has_key('expCpsInfo'):
                ...
                if not self.check_cps_info(item['expCpsInfo'],item['resCpsInfo']):
                    ...
                    self.warnList.append(dsc)
            elif not item.has_key('expCpsInfo'):
                dsc = '商家【'+str(item['name'])+'】：cps_info 不一致，请给出预期cps_info。'
                self.warnList.append(dsc)
            elif not item.has_key('resCpsInfo'):
                dsc = '商家【'+str(item['name'])+'】：cps_info 不一致，预期：'+str(item['expCpsInfo'])+"，实际返回无cps_info"
                self.warnList.append(dsc)
